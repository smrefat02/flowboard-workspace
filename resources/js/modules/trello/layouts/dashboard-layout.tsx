...
import { Bell, BriefcaseBusiness, Plus, Search, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen px-3 py-4 md:px-6 md:py-6">
      <div className="mx-auto grid max-w-[1500px] grid-cols-1 gap-4 lg:grid-cols-[300px_1fr]">
        <aside className="glass fade-up rounded-[28px] border border-slate-200/80 p-5 md:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Workspace switcher</p>
            </div>
          </div>
            ...
            {workspaces.map((workspace) => (
              <div key={workspace.id} className="group flex items-center gap-1.5">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="h-8 shrink-0 rounded-lg border-slate-200 bg-white px-2 text-xs font-medium text-slate-600 hover:text-slate-900"
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-8 w-8 shrink-0 rounded-lg p-0 text-slate-400 opacity-100 hover:bg-red-50 hover:text-red-600"
          ...
          {activeWorkspace ? (
            <Button
              className="mt-4 h-9 w-full justify-start rounded-xl border-slate-200 bg-white text-slate-700 hover:bg-slate-100"
              variant="outline"
          ...
          <div className="mt-6 rounded-2xl border border-sky-200/80 bg-sky-50/80 p-3 text-xs leading-relaxed text-sky-900">
            Drag lists and cards to reprioritize instantly.
          </div>
        </aside>

        <main className="space-y-4">
          <header className="glass fade-up flex flex-wrap items-center justify-between gap-3 rounded-[26px] border border-slate-200/80 px-4 py-3 md:flex-nowrap md:px-5">
            <div className="flex w-full max-w-xl items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-500 shadow-sm">
            ...
            <Button variant="outline" size="sm" className="rounded-xl border-slate-200 bg-white px-3.5">
              <Bell className="mr-1 h-4 w-4" /> Notifications
            </Button>
            <Button
              variant={compactMode ? 'default' : 'outline'}
              size="sm"
              className={`rounded-xl px-3.5 ${compactMode ? '' : 'border-slate-200 bg-white'}`}
